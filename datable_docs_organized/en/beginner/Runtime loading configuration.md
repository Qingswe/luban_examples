...
> 来源: https://www.datable.cn/en/docs/beginner/loadinruntime
...
We take the most common Unity + c# + json as an example, the sample project is [Csharp_Unity_Json](https://github.com/focus-creative-games/luban_examples/tree/main/Projects/Csharp_Unity_json), For other types, please refer to the corresponding projects in the Projects directory.
...
Please connect Luban according to the [Connect Luban to Project](Integrate Luban To Your Project.md) document, and prepare the corresponding generation script gen.bat. If you have any questions, you can refer to the `gen.bat` file of the `Csharp_Unity_Json` project.
...
You can support asynchronous loading by modifying the [code template](../manual/custom template.md).

## Use configuration​

cfg.Tables contains an instance field of all configuration tables. After loading cfg.Tables, use `tables.<table name>` to get the table instance, and then perform subsequent operations on the table. For example, we want to print the reward information of the Reward table id = 1001, the code is as follows:
...
You may notice that the first letter of the reward table field names id, name, and desc are capitalized. This is because the tool will automatically convert the field names to the corresponding code style according to the output language, that is, boo_bar will be converted to a name like BooBar. Therefore, it is recommended to use the xx_yy_zz style when configuring the field names.
