...
> 来源: https://www.datable.cn/en/docs/3.x/manual/commandtools
...
Most of Luban's built-in templates use the [Cascading Option](Hierarchical parameter mechanism.md) mechanism. You only need to use the `<code target name>.outputCodeDir` parameter Just specify the output directory parameters for each target separately.
...
namingConvention.{codeTarget}.{location}| codeTarget is the target name specified in the `--codeTarget` parameter. location is the style location. Optional values ​​are namespace, type, method, property, field, enumItem. For details, see [Code Style](Code Style.md). This parameter is a hierarchical option. If {codeTarget} is not specified, it will take effect on all targets| none, pascal, camel, upper, snake| -x namingConvention.cs-bin.field=pascal  
