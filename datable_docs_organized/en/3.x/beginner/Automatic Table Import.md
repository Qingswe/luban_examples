...
> 来源: https://www.datable.cn/en/docs/3.x/beginner/importtable
...
Automatic import supports custom import rules. For details, please refer to the document [Automatic table import](../manual/Automatic table import.md).

## Create an automatically imported table​

Copy the reward.xlsx created in [Quick Start](Quick Start.md) as `#Reward2.xlsx` file. **No** modification of __tables__.xlsx is required. Regenerate it and you will find that the TbReward2 table has been added, and the table record type is Reward2.

For common situations, this way of adding tables is very convenient.

## Default import rules​

By default, all excel (xls, xlsx, xlm, csv) files with a file name starting with # in the configuration directory (dataDir field in luban.conf) (including subdirectories) will be scanned, such as `#Item.xlsx`, `reward/#Reward.xlsx`. The string after removing the leading '#' character and the file suffix of the file name is used as the value_type of the table, and Tb is added to the value_type name as the full_name of the table. If the excel file is in a subdirectory, the subdirectory will be used as the namespace. For example:
