...
> 来源: https://www.datable.cn/en/docs/3.x/help/faq
...
The index field of the table specifies the list of primary keys. For details, please refer to the relevant documents about the mode and index of the table in [Configuration Related Definitions](../manual/schema logical structure.md).
...
support. When table mode=list, joint multi-primary key mode and independent multi-primary key mode are supported. For detailed documents, please refer to the related documents about table mode in [configuration related definitions](../manual/schema logical structure.md).

## Does it support exporting different tables and fields by client and server?​

support. [Configuration related definitions](../manual/schema logical structure.md) Related documents about classification definition and group export.
...
Can. See [configuration-related definitions](../manual/schema logical structure.md) Documentation on table.input in . 
...
Use a catalog data source. Put all json files in a directory (it can be a directory tree), and set input to this directory. luban will automatically traverse the entire directory tree, treating each file as A record is read in. For details, see [Other Data Sources-json](../manual/Non-excel data format.md)

## Can a json file contain multiple records?​

Can. But it must be specified in the form of *@xxx.json in the data source. For details, see [Other Data Sources-json](../manual/Non-excel data format.md)

## Can a record come from some deep field of a json file?​

Can. There are two situations:

  * To read a record from a field, specify it in the form of [a.b.c@xx.json](mailto:a.b.c@xx.json)
  * Read the list of records from the field, specify in the form of *[a.b.c@xx.json](mailto:a.b.c@xx.json)



For details, see [Other Data Sources-json](../manual/Non-excel data format.md)

## Can the data of multiple tables be put into one json file like xlsx?​

Can. Similar to the excel data source, as long as each table is specified in the form of [field@xx.json](mailto:field@xx.json) or *[field@xx.json.](mailto:field@xx.json.) For details, see [Other Data Sources-json](../manual/Non-excel data format.md)
...
Yes, enumerations and structures of external types are supported, but currently only the C# language is supported. For detailed documentation, see the type mapper related documentation in [Configuration Definition Introduction](../manual/schema logical structure.md).
