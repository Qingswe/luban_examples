...
> 来源: https://www.datable.cn/en/docs/1.x/manual/define
...
目前已经定义了一些内部option
  ...
  * cpp _bin.stub_file_name_format cpp_bin stub文件的格式。默认 gen_stub_{0}.cpp
  * cpp_ue_editor_json.type_per_stub_file cpp_ue_editor_json 每个stub文件包含的文件个数，默认200
  * cpp _ue_editor_json.stub_file_name_format cpp_ue_editor_json的stub文件名格式，默认gen_stub_{0}.cpp
...
目前支持的检验器有ref,path,range,set,size，详细请参见文档 [数据校验器](检验器.md)。
...
注意与定义标签不同。定义作用于结构，字段，表等等定义上，数据标签作用于记录。详细文档见 [**tag**](tag 介绍.md)
...
由于策划填的都是 墙上时间（时期跟时区相关），所以导出时需要指定 datetime所对应的时区，不指定则默认使用东8区时区。 参见 [Luban 命令行](命令行工具.md) 中关于时区相关的参数介绍。
...
请参见 [luban_examples](https://gitee.com/focus-creative-games/luban_examples)/DataTables目录下的 __enums__.xlsx, __beans__.xlsx, __tables__.xlsx。
