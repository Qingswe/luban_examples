...
支持以下几类本地化：
  ...
  * [字段变体](/docs/manual/variants)。为字段的每个本地化目标配置一个变体字段，导出时只导出指定变体的数据
...
本地化数据文件可以是excel格式，也可以是json、xml之类的格式，只要它符合luban的builtin DataLoader的格式标准（见[excel格式](/docs/manual/excel)和[非excel格式](/docs/manual/otherdatasource)文档）即可。 当使用json格式时，由于不可能在一个json文件中只放一个text数据，实践中会让json文件中包含text列表，可以用复合数据文件格式（见[非excel格式](/docs/manual/otherdatasource)） 填。以下为示例json格式的本地化文本表，通过`-x l10n.textFile.path=*@your_text_file.json` 指定读入text列表。
...
详见[字段变体](/docs/manual/variants)。

## 收集配置表中所有text的列表​

有时候你想收集配置中所有的本地化text的key，DataTarget text-list 正用于此目的。text-list会收集配置表中出现的所有text数据，按从小到大排序后，输出文件。 通过`l10n.textListFile`选项指定输出文件，详细见[命令行工具](/docs/manual/commandtools)文档。
