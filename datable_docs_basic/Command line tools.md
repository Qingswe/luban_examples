...
> 来源: https://www.datable.cn/en/docs/manual/commandtools
...
\--variant| No| | Specify the field variant to be used, the format is `--variant {variantKey}={variantName}`, where `{variantKey}` is `{beanFullName}.{fieldName}` (such as `test.TestVariant.value`). There can be multiple `--variant` to specify variants for different fields. For detailed documentation, see [variant variants](/en/docs/manual/variants)  
...
Most of Luban's built-in templates use the [Cascading Option](/en/docs/manual/cascadingoption) mechanism. You only need to use the `<code target name>.outputCodeDir` parameter Just specify the output directory parameters for each target separately.
...
namingConvention.{codeTarget}.{location}| codeTarget is the target name specified in the `--codeTarget` parameter. location is the style location. Optional values ​​are namespace, type, method, property, field, enumItem. For details, see [Code Style](/en/docs/manual/codestyle). This parameter is a hierarchical option. If {codeTarget} is not specified, it will take effect on all targets| none, pascal, camel, upper, snake| -x namingConvention.cs-bin.field=pascal  
